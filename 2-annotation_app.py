...
def main():
    ...
    if record_index < len(df):
        ...
        with middle_col:
            ...
            # Try to get 'Final Post' column, fallback to 'LLM Response'
            final_post = current_record.get('Final Post', None)
            if final_post is not None and str(final_post).strip() != '':
                st.text_area(
                    "Final Post Content:",
                    value=str(final_post),
                    height=400,  # More space for longer responses
                    key=f"final_post_{record_index}",
            ...
            # Add expander for Writing Steps if available
            writing_steps = current_record.get('Writing Steps', None)
            if writing_steps is not None and str(writing_steps).strip() != '':
                with st.expander("Show Writing Steps"):
                    st.text_area(
                        "Writing Steps:",
                        value=str(writing_steps),
                        height=300,
                        key=f"writing_steps_{record_index}",
        ...
        if st.button("📥 Download Annotations"):
            ...
            export_data = {
                ...
                'writing steps': df.get('Writing Steps', ''),
                'final post': df.get('Final Post', ''),
